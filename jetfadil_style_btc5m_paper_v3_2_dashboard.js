...
// ─────────────────────────────────────────────────────────────────────────────
// Railway Web Dashboard Add-on
// NOTE: This section only exposes the current bot state/status through HTTP.
// It does NOT change strategy rules, entry logic, settlement logic, or paper
// trading calculations.
// ─────────────────────────────────────────────────────────────────────────────

const DASHBOARD_PORT = Number(process.env.PORT || argv.port || 3000);
...
function dashboardPayload() {
  ...
  return {
    updatedAt: nowIso(),
    title: BOT_TITLE,
    sessionElapsed: fmtMmSs(sessionElapsedSec()),
    slug: state.slug || '-',
    marketLabel: state.marketLabel || '-',
    displaySlug: state.slug ? displaySlugLabel(state.slug, state.marketLabel) : '-',
    elapsed: timer ? fmtMmSs(timer.elapsedSec) : '-',
    remaining: timer ? fmtMmSs(timer.remainingSec) : '-',
    upPrice: Number.isFinite(upPx) ? upPx : null,
    downPrice: Number.isFinite(downPx) ? downPx : null,
    leader: state.strategy.leader || null,
    laggard: state.strategy.laggard || null,
    anchor: state.strategy.anchorSide || null,
    score: state.strategy.score || 0,
    status: state.strategy.status || '-',
    reason: state.strategy.reason || '-',
    lastAction: state.strategy.lastAction || '-',
    momentum: {
      edge: Number.isFinite(parts.edge) ? parts.edge : null,
      leaderM20: Number.isFinite(parts.leaderM20) ? parts.leaderM20 : null,
      leaderM45: Number.isFinite(parts.leaderM45) ? parts.leaderM45 : null,
      laggardM20: Number.isFinite(parts.laggardM20) ? parts.laggardM20 : null,
      streak: fmtMmSs(state.slugState.leaderStreakSec || 0),
      flipsUsed: state.slugState.flipsUsed || 0,
      allowFlip: CONFIG.allowFlip,
    },
    balance: {
    ...
    stats: {
      ...
      totalSettled: state.paper.totalSettled,
      wins: state.paper.wins,
      losses: state.paper.losses,
    },
    openLots,
    activeTotals: totalsBySide(openLots),
    currentSlugOpenPnl: currentSlugOpenPnl(openLots),
    recentActions: state.recentActions.slice(-20).reverse(),
    slugHistory: (state.paper.slugHistory || []).slice(-60),
    logLines: state.logLines.slice(-20),
    statusText: safeReadText(state.paths.statusFile, 'Status file not ready yet.'),
    summaryText: safeReadText(state.paths.summaryFile, 'Summary file not ready yet.'),
  };
}
...
<head>
  ...
  <style>
    :root{--bg:#080b12;--card:#111827;--card2:#0f172a;--muted:#94a3b8;--text:#e5e7eb;--line:#243044;--green:#22c55e;--red:#ef4444;--blue:#38bdf8;--yellow:#facc15;}
    *{box-sizing:border-box} body{margin:0;background:linear-gradient(135deg,#020617,#0f172a 60%,#111827);color:var(--text);font-family:Inter,system-ui,Segoe UI,Arial,sans-serif;}
    ...
    .pill{border:1px solid var(--line);border-radius:999px;padding:8px 12px;background:rgba(15,23,42,.72);font-size:13px;color:var(--muted)}
    .grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}
    .wide{grid-column:span 2}.full{grid-column:1/-1}
    .card{background:rgba(17,24,39,.88);border:1px solid var(--line);border-radius:18px;padding:16px;box-shadow:0 12px 30px rgba(0,0,0,.28)}
    .label{color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.08em}.value{font-size:25px;font-weight:750;margin-top:6px}.small{font-size:13px;color:var(--muted);line-height:1.45}
    .green{color:var(--green)}.red{color:var(--red)}.blue{color:var(--blue)}.yellow{color:var(--yellow)}
    .status{font-size:16px;font-weight:700}.reason{margin-top:8px;color:var(--muted);font-size:13px}
    table{width:100%;border-collapse:collapse;font-size:13px}th,td{padding:9px;border-bottom:1px solid var(--line);text-align:left}th{color:var(--muted);font-weight:600}
    pre{white-space:pre-wrap;overflow:auto;background:#020617;border:1px solid var(--line);border-radius:14px;padding:14px;max-height:420px;font-size:12px;color:#cbd5e1}
    .bar{height:10px;border-radius:999px;background:#020617;border:1px solid var(--line);overflow:hidden}.bar span{display:block;height:100%;background:linear-gradient(90deg,#38bdf8,#22c55e)}
    .hist{display:flex;gap:6px;align-items:end;height:120px;border-bottom:1px solid var(--line);padding-top:12px;overflow-x:auto}.hist div{min-width:12px;border-radius:4px 4px 0 0;background:var(--blue)}.hist div.neg{background:var(--red)}.hist div.pos{background:var(--green)}
    @media(max-width:900px){.grid{grid-template-columns:1fr}.wide{grid-column:auto}.wrap{padding:12px}}
  </style>
</head>
<body>
  <div class="wrap">
    <div class="top">
      <div><h1>JetFadil BTC 5m Dashboard</h1><div class="sub" id="displaySlug">Loading...</div></div>
      <div class="pill">Auto refresh: 1s · Railway PORT ready</div>
    </div>

    <div class="grid">
      <div class="card"><div class="label">Status</div><div class="value blue" id="status">-</div><div class="reason" id="reason">-</div></div>
      <div class="card"><div class="label">Timer</div><div class="value" id="timer">-</div><div class="small">Elapsed / Rollover</div></div>
      <div class="card"><div class="label">Prices</div><div class="value"><span class="green" id="up">-</span> / <span class="red" id="down">-</span></div><div class="small">UP / DOWN mid</div></div>
      <div class="card"><div class="label">Bias</div><div class="value" id="bias">-</div><div class="small" id="bias2">-</div></div>

      <div class="card"><div class="label">Available</div><div class="value" id="available">-</div></div>
      <div class="card"><div class="label">Locked</div><div class="value yellow" id="locked">-</div></div>
      <div class="card"><div class="label">Realized P&L</div><div class="value" id="realized">-</div></div>
      <div class="card"><div class="label">Stats</div><div class="value" id="stats">-</div></div>

      <div class="card wide"><div class="label">Current Slug Open P&L</div><div class="value" id="openPnl">-</div><div class="bar"><span id="scoreBar" style="width:0%"></span></div><div class="small" id="momentum">-</div></div>
      <div class="card wide"><div class="label">Last Action</div><div class="value" id="lastAction">-</div><div class="small" id="updated">-</div></div>

      <div class="card full"><div class="label">Slug P&L History</div><div class="hist" id="hist"></div></div>

      <div class="card full"><div class="label">Open Lots</div><div id="openLots"></div></div>
      <div class="card full"><div class="label">Recent Actions</div><div id="actions"></div></div>
      <div class="card full"><div class="label">Live Status File</div><pre id="statusText">Loading...</pre></div>
    </div>
  </div>

<script>
const money = n => Number.isFinite(Number(n)) ? '$' + Number(n).toFixed(2) : '-';
const cents = n => Number.isFinite(Number(n)) ? Math.round(Number(n)*100) + 'c' : '-';
const pnlText = n => (Number(n) >= 0 ? '+' : '-') + '$' + Math.abs(Number(n||0)).toFixed(2);
const pnlClass = n => Number(n) >= 0 ? 'green' : 'red';
const sideClass = s => String(s||'').toLowerCase()==='up' ? 'green' : String(s||'').toLowerCase()==='down' ? 'red' : '';
...
async function refresh(){
  try{
    ...
    document.getElementById('displaySlug').textContent = s.displaySlug;
    document.getElementById('status').textContent = s.status;
    document.getElementById('reason').textContent = s.reason;
    document.getElementById('timer').textContent = s.elapsed + ' / ' + s.remaining;
    document.getElementById('up').textContent = cents(s.upPrice);
    document.getElementById('down').textContent = cents(s.downPrice);
    document.getElementById('bias').innerHTML = '<span class="'+sideClass(s.leader)+'">'+(s.leader||'-').toUpperCase()+'</span> · score '+s.score;
    document.getElementById('bias2').textContent = 'Anchor: ' + ((s.anchor||'-').toUpperCase());
    document.getElementById('available').textContent = money(s.balance.availableCash);
    document.getElementById('locked').textContent = money(s.balance.lockedCapital);
    const realized = document.getElementById('realized'); realized.textContent = pnlText(s.balance.realizedPnl); realized.className = 'value '+pnlClass(s.balance.realizedPnl);
    document.getElementById('stats').textContent = s.stats.totalBuys + ' buys · ' + s.stats.wins + 'W/' + s.stats.losses + 'L';
    const op = document.getElementById('openPnl'); op.textContent = pnlText(s.currentSlugOpenPnl); op.className = 'value '+pnlClass(s.currentSlugOpenPnl);
    document.getElementById('scoreBar').style.width = Math.min(100, Math.max(0, Number(s.score||0)*12)) + '%';
    document.getElementById('momentum').textContent = 'edge=' + cents(s.momentum.edge) + ' · streak=' + s.momentum.streak + ' · flips=' + s.momentum.flipsUsed;
    document.getElementById('lastAction').textContent = s.lastAction;
    document.getElementById('updated').textContent = 'Updated: ' + s.updatedAt + ' · Session: ' + s.sessionElapsed;
    document.getElementById('openLots').innerHTML = table(s.openLots, [
      ['Side', x=>'<span class="'+sideClass(x.side)+'">'+String(x.side).toUpperCase()+'</span>'],
      ['Qty', x=>Number(x.qty||0).toFixed(4)],
      ['Entry', x=>cents(x.entryPrice)],
      ['USD', x=>money(x.entryUsd)],
      ['Source', x=>x.source||'-'],
      ['Reason', x=>x.reason||'-']
    ]);
    document.getElementById('actions').innerHTML = table(s.recentActions, [
      ['Kind', x=>x.kind||'-'],
      ['Side', x=>'<span class="'+sideClass(x.side)+'">'+String(x.side||'-').toUpperCase()+'</span>'],
      ['Price', x=>cents(x.price)],
      ['USD', x=>money(x.usd)],
      ['Source', x=>x.source||'-'],
      ['Reason', x=>x.reason||'-']
    ]);
    const hist = document.getElementById('hist'); hist.innerHTML = '';
    const vals = (s.slugHistory||[]).map(x=>Number(x.pnl||0));
    const max = Math.max(1, ...vals.map(v=>Math.abs(v)));
    vals.forEach(v=>{
      const d=document.createElement('div');
      d.className = v>=0 ? 'pos':'neg';
      d.title = pnlText(v);
      d.style.height = (8 + Math.abs(v)/max*100) + 'px';
      hist.appendChild(d);
    });
    if(!vals.length) hist.innerHTML = '<div class="small">No settled slug history yet.</div>';
...
function sendDashboardResponse(res, statusCode, contentType, body) {
  ...
  dashboardServer = http.createServer((req, res) => {
    try {
      ...
      if (url.pathname === '/health') {
        return sendDashboardResponse(res, 200, 'application/json; charset=utf-8', JSON.stringify({ ok: true, updatedAt: nowIso() }));
      }
      if (url.pathname === '/api/state') {
        return sendDashboardResponse(res, 200, 'application/json; charset=utf-8', JSON.stringify(dashboardPayload()));
      }
      if (url.pathname === '/api/status.txt') {
        return sendDashboardResponse(res, 200, 'text/plain; charset=utf-8', safeReadText(state.paths.statusFile, 'Status file not ready yet.'));
      }
      return sendDashboardResponse(res, 200, 'text/html; charset=utf-8', dashboardHtml());
    } catch (err) {
      return sendDashboardResponse(res, 500, 'text/plain; charset=utf-8', `Dashboard error: ${err.message}`);
    }
  });

  dashboardServer.listen(DASHBOARD_PORT, '0.0.0.0', () => {
    logEvent(`DASHBOARD_READY | port=${DASHBOARD_PORT} | health=/health | api=/api/state`, state.slug);
  });
